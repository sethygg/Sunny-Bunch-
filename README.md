...
## Mia’s Place mission expansion

The founder broadened the mission on September 22, 2026: help communities support people with special needs and their families through school support, responsible treatment research, and meaningful employment. Parents and communities should have practical resources and greater confidence in belonging, purpose and long-term security. The homepage includes the three priorities, family-support vision and a broader phased plan. School/influencer pages, sharing drafts and the AI management brief use the same purpose. Initiatives remain planned; no services, research grants, treatment outcomes or guaranteed security are asserted.

Canonical short mission copy lives in `lib/mission-copy.mjs`. The saved catalog already held the earlier employment-focused defaults, so `snapshot()` performs one narrowly scoped, audited update of those exact legacy values saved before the release cutoff. Custom text and later owner edits remain authoritative. The transaction increments the store revision and uses an operation guard; existing stale proposals must be refreshed. No prices, inventory, partner attribution, donation records or giving commitments are changed. The update is idempotent and rolls back if auditing fails. No schema migration is needed.

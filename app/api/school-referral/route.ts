...
export async function GET(request:Request){try{const selected=await resolveSchoolReferral(referralToken(request));return json({school:selected?.kind==='school'?publicSchool(selected):null,influencer:selected?.kind==='influencer'?{code:selected.code,name:selected.name}:null,expiresAt:selected?.expires_at??null});}catch(error){return errorResponse(error);}}
